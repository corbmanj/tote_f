  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return switch (itemsList) {
      AsyncData(value: final items) => Column(
        children: [
          // Optional: Add a page title here
          Expanded(
            child: SplitView(
                        ...
                        child: ListView.separated(
                            ...
                            itemBuilder:
                                (BuildContext context, int itemIndex) =>
                                    ItemChip(
                                        item: items.userItemsAndGroups
                                            .userItems[itemIndex]),
                            separatorBuilder:
                                (BuildContext context, int outfitIndex) =>
                                    SizedBox(width: 30.0),
                            itemCount:
                                items.userItemsAndGroups.userItems.length),
                      ),
                    ),
                  ],
                ),
                                ...
                                (BuildContext context, int itemIndex) =>
                                    AdditionalItemChip(
                                        item: items
                                            ...
                                            .userAdditionalItems[itemIndex]),
                            separatorBuilder:
                                (BuildContext context, int outfitIndex) =>
                                    SizedBox(height: 8.0),
                            itemCount: items.userAdditionalItemsAndSections
                                .userAdditionalItems.length),
                      ),
                    ),
                  ],
                ),
